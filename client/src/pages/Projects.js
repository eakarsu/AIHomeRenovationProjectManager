...
export default function Projects({ user, onLogout }) {
  ...
  useEffect(() => { load(); }, []);
  const load = async () => { const { data } = await API.get('/projects'); setItems(data); };

  const handleSave = async () => {
    if (editing) await API.put(`/projects/${form.id}`, form);
    else await API.post('/projects', form);
    setShowForm(false); setForm(empty); setEditing(false); load();
  };
  const handleDelete = async (id) => { if (!window.confirm('Delete?')) return; await API.delete(`/projects/${id}`); setSelected(null); load(); };
  ...
  const handleAI = async () => { setAILoading(true); setAIData(null); setShowAI(true); try { const { data } = await API.post('/projects/ai/analyze'); setAIData(data); } catch(e) { setAIData({success:false,content:e.message}); } setAILoading(false); };
  ...
  return (
    <Layout user={user} onLogout={onLogout}>
      ...
      <div className="summary-grid">
        <div className="summary-card"><div className="label">Total Projects</div><div className="value">{items.length}</div></div>
      ...
      <div className="data-table-container"><table className="data-table"><thead><tr>
        ...
        {items.map(item => (
          <tr key={item.id} onClick={() => setSelected(item)}>
            ...
            <td className="money">${parseFloat(item.total_budget).toLocaleString()}</td>
      ...
      {selected && <div className="modal-overlay" onClick={()=>setSelected(null)}><div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div className="detail-grid">
            ...
            <div className="detail-item"><div className="label">Budget</div><div className="value money">${parseFloat(selected.total_budget).toLocaleString()}</div></div>
